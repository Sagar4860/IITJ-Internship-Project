...
def is_chinese(word):
    ...
    return all('\u4e00' <= ch <= '\u9fff' and unicodedata.category(ch).startswith('Lo') for ch in word)
import re

def remove_english(text_list):
    words = []
    for text in text_list:
        # Check if the text contains any Latin letters (English)
        if not re.search(r'[a-zA-Z]', text):
            words.append(text)
    return words


def get_words(lang,leng,number_of_words):
    all_words = top_n_list(language[lang], 1000000)
    filtered_words = [word for word in all_words if len(word) > leng]
    if(lang in ['japanese','hindi','tamil','telugu','bengali','chinese',]): filtered_words = remove_english(filtered_words)
    return filtered_words[:number_of_words]

def get_chinese_words(lang,leng,number_of_words):
    all_words = top_n_list(language[lang], 3000000)
    filtered_words = [word for word in all_words if is_chinese(word) and len(word) >= leng]
    return filtered_words[:number_of_words]
def is_arabic(word):
    return all(
        ('\u0600' <= ch <= '\u06FF') or
...
def get_arabic_words(lang, leng, number_of_words):
    all_words = top_n_list(language[lang], 3000000)  # Your language word source
    filtered_words = [word for word in all_words if is_arabic(word) and len(word) >= leng]
    return filtered_words[:number_of_words]

def pure_numbers(number):
    pure_numberss = [str(random.randint(100, 99999)) for _ in range(number)]
    return pure_numberss

def mix_words(mix_base_words):
    mixed_words = []
...
def load_vocab(lang,size=20000, base_dir=f"/DATA1/ocrteam/word_restoration/reconstruction/datasets/vocabulary"):
    file_path = f"{base_dir}/{lang.lower()}_words_200k.txt"
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            words = [line.strip() for line in f if line.strip() and len(line.strip()) >=3]
            return words[:size]
    except FileNotFoundError:
        print(f"File not found for language: {lang}")
        return []

hindi_words = get_words("hindi",5,numberofsample)
hindi_words += pure_numbers(onlynumber)
hindi_words+=mix_words(hindi_words[:onlynumber])

japanese_words = get_words("japanese",5,numberofsample)

japanese_words += pure_numbers(onlynumber)
japanese_words+=mix_words(japanese_words[:onlynumber])

chinese_words = get_chinese_words("chinese",3,numberofsample)
chinese_words += pure_numbers(onlynumber)
chinese_words+=mix_words(chinese_words[:onlynumber])

german_words = get_words("german",5,numberofsample)
german_words += pure_numbers(onlynumber)
german_words+=mix_words(german_words[:onlynumber])

arabic_words = get_arabic_words('arabic',4,numberofsample)
arabic_words += pure_numbers(onlynumber)
arabic_words += mix_words(arabic_words[:onlynumber])

french_words = get_words("french", 5, numberofsample)
french_words += pure_numbers(onlynumber)
french_words += mix_words(french_words[:onlynumber])

telugu_words = load_vocab("telugu")
telugu_words += mix_words(telugu_words[:onlynumber])
telugu_words += pure_numbers(onlynumber)

tamil_words = get_words("tamil",5,numberofsample)
tamil_words += pure_numbers(onlynumber)
tamil_words += mix_words(tamil_words[:onlynumber])

bengali_words = get_words("bengali",5,numberofsample)
bengali_words += pure_numbers(onlynumber)
bengali_words += mix_words(bengali_words[:onlynumber])

sanskrit_words = load_vocab("sanskrit")
sanskrit_words += mix_words(sanskrit_words[:onlynumber])
sanskrit_words += pure_numbers(onlynumber)

malayalam_words = load_vocab("malayalam")
malayalam_words += mix_words(malayalam_words[:onlynumber])
malayalam_words += pure_numbers(onlynumber)

punjabi_words = load_vocab("punjabi")
punjabi_words += mix_words(punjabi_words[:onlynumber])
punjabi_words += pure_numbers(onlynumber)

marathi_words = load_vocab("marathi")
marathi_words += mix_words(marathi_words[:onlynumber])
marathi_words += pure_numbers(onlynumber)

gujarati_words = load_vocab("gujarathi")
gujarati_words += mix_words(gujarati_words[:onlynumber])
gujarati_words += pure_numbers(onlynumber)




filtered_words = get_words("english",5,20000)
random.shuffle(filtered_words)  # Shuffle at the start to randomize selection
# Step 2: Slice unique segments for each type
lower_words = filtered_words[10000:11000] #50% of data
upper_words = [word.upper() for word in filtered_words[11000:11200]] #10% of data
capitalized_words = [word.capitalize() for word in filtered_words[11200:11600]] #20% 0f data
mix_base_words = filtered_words[11600:11800] #num+words 10%of data
mix_base_words = mix_words(mix_base_words)

english_words = lower_words + upper_words + capitalized_words + mix_base_words + pure_numbers(200)
random.shuffle(english_words)


languages = [
    'Telugu','English', 'Arabic', 'Chinese', 'French', 'German', 'Hindi', 'Japanese',
    'Tamil', 'Bengali', 'Sanskrit', 'Malayalam', 'Punjabi', 'Marathi', 'Gujarati'
]
...
def generate_perfectly_cropped_image(
    ...
    try:
        ...
        print(f"⚠ Error getting bounding box for text: {text[:30]}... | {type(e).__name__}: {e}")
        return False, 0
...
def generate_dataset_pil(
    ...
    for phrase in phrases:
        ...
        for i in range(samples_per_phrase):
            sample_id = f"{cnt:03d}"
            clean_path = os.path.join(clean_output_dir, f"{sample_id}.png")

            bitmask_path = os.path.join(bitmask_output_dir, f"{sample_id}.png")
            ISM_path=os.path.join(ISM_output_dir, f"{sample_id}.png")
            CSM_path=os.path.join(CSM_output_dir, f"{sample_id}.png")
            ...
            noisy_path = os.path.join(noisy_dir, f"{sample_id}.png")
            ...
            elif re.search(r'[A-Za-z\u0900-\u097F\u3040-\u30FF\u4e00-\u9fff]', phrase) and re.search(r'\d', phrase):
                word_type = "mixed"
...
metadata_csv_path = os.path.join(output_root_csv, "synthetic_image_test.csv")
