...
def run_ocr():
    ...
    metadata['output_path'] = metadata['clean_path'].apply(lambda x: x.replace('/clean/', '/output_BSTD/'))

    count = 0


    for idx, row in tqdm(metadata.iterrows(), total=len(metadata), desc="🔍 Running OCR"):
        count+=1
